...
local Library = {
    ...
    DPIScale = 1,
    CornerRadius = 8,
    CornerRadiusDropdown = true, -- Enabled: rounded dropdown containers
...
local Templates = {
    ...
    Dropdown = {
        Values = {},
        DisabledValues = {},
        ValueImages = {}, -- [Value] = "rbxassetid://XXXX" | number | lucide icon name
...
do
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        local DisplayButton = New("TextButton", {
            ...
            BackgroundTransparency = 1,
            RichText = true, -- Allows <b>, <font color>, etc. in dropdown display text
            Size = UDim2.new(1, 0, 0, 21),
            Text = "---",
            TextSize = 14,
            TextXAlignment = Enum.TextXAlignment.Left,
        ...
        -- Corner radius is applied when Library.CornerRadiusDropdown == true (default: true)
        ...
        local MenuTable = Library:AddContextMenu(
            ...
            not Library.CornerRadiusDropdown -- pass IgnoreCornerRadius = false when CornerRadiusDropdown is true
        )
        ...
        function Dropdown:Display()
            ...
            -- Strip rich text tags for length measurement so tags don't get truncated mid-way
            local PlainStr = Str:gsub("<[^>]+>", "")
            if #PlainStr > 25 then
                -- Truncate the plain version; if RichText is used just show ellipsis safely
                if Str ~= PlainStr then
                    Str = PlainStr:sub(1, 22) .. "..."
                else
                    Str = Str:sub(1, 22) .. "..."
                end
            end
